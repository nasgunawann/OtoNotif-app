...
import { IconBell, IconTool, IconDroplet, IconAlertCircle, IconCircleCheck, IconGauge, IconReceipt } from "@tabler/icons-react"
...
export function NotificationSheet({ children }: { children: React.ReactNode }) {
  const { notifications, fetchNotifications, markNotificationsAsRead } = useVehicleStore()
        ...
        <ScrollArea className="flex-1">
          {notifications.length > 0 ? (
            <div className="divide-y">
              {notifications.map((n) => {
                const Icon = iconComponents[n.icon as keyof typeof iconComponents] || IconAlertCircle
