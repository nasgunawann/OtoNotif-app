...
interface VehicleStore {
  ...
  createVehicle: (
    data: Partial<Vehicle> & { name: string; type: "motor" | "mobil" },
  ) => Promise<Vehicle>;
...
export const useVehicleStore = create<VehicleStore>((set, get) => ({
  ...
  createVehicle: async (data) => {
    ...
    try {
      ...
      set((s) => ({
        vehicles: s.vehicles.map((v) => (v.id === tempId ? vehicle : v)),
      }));
      return vehicle;
    } catch (e) {
  ...
  setPrimaryVehicle: async (id: string) => {
    ...
    const previousVehicles = state.vehicles;
    set({
      vehicles: previousVehicles.map((v) => ({ ...v, isPrimary: v.id === id })),
    });
    try {
  ...
  updateVehicle: async (id: string, data: Partial<Vehicle>) => {
    ...
    set({
      ...
      selectedVehicle:
        previousSelected && previousSelected.id === id
    ...
    try {
      ...
      set((s) => ({
        ...
        selectedVehicle:
          s.selectedVehicle?.id === id ? updated : s.selectedVehicle,
      }));
    } catch (e) {
  ...
  createOdometerReading: async (data) => {
    ...
    if (
      previousHealth &&
      previousHealth.vehicle.id === data.vehicleId &&
      data.reading > (previousHealth.latestOdo || 0)
    ) {
      set({
        vehicleHealth: {
          ...previousHealth,
          ...
          components: previousHealth.components.map((c) => {
            ...
            const usagePercent = Math.min(
              100,
              (usedKm / c.component.intervalKm) * 100,
            );
            const status =
              usagePercent > 85
                ? ("danger" as const)
    ...
    try {
      ...
      set((s) => ({
        odometerReadings: s.odometerReadings.map((r) =>
          r.id === tempId ? reading : r,
        ),
      }));
  ...
  createFuelLog: async (data) => {
    ...
    if (
      ...
      previousHealth &&
      previousHealth.vehicle.id === data.vehicleId &&
      data.odoReading > (previousHealth.latestOdo || 0)
    ) {
      set({
        vehicleHealth: {
          ...previousHealth,
          ...
          components: previousHealth.components.map((c) => {
            const usedKm =
              data.odoReading! - (c.component.lastReplacedOdo ?? 0);
            const remainingKm = Math.max(0, c.component.intervalKm - usedKm);
            const usagePercent = Math.min(
              100,
              (usedKm / c.component.intervalKm) * 100,
    ...
    try {
      ...
      set((s) => ({
        fuelLogs: s.fuelLogs.map((l) => (l.id === tempId ? log : l)),
      }));
      get().fetchVehicleHealth(data.vehicleId);
    } catch (e) {
  ...
  createComponent: async (data) => {
    ...
    try {
      ...
      set((s) => ({
        components: s.components.map((c) => (c.id === tempId ? component : c)),
      }));
      get().fetchVehicleHealth(data.vehicleId);
    } catch (e) {
      ...
      set((s) => ({
        components: s.components.map((c) => {
          const match = results.find(
            (r) => r.name === c.name && c.id.startsWith("temp-"),
          );
          return match || c;
        }),
      }));
        ...
        vehicleHealth: {
          ...
          components: previousHealth.components.map((c) =>
            c.component.id === data.componentId
              ? {
                  component: { ...c.component, lastReplacedOdo: currentOdo },
                  currentOdo,
                  usedKm: 0,
    ...
    try {
      ...
      set((s) => ({
        maintenanceRecords: s.maintenanceRecords.map((r) =>
          r.id === tempId ? record : r,
        ),
      }));
      get().fetchVehicleHealth(data.vehicleId);
    } catch (e) {
      set({
        maintenanceRecords: previousRecords,
        vehicleHealth: previousHealth,
      });
      throw e;
    }
